...
public class Continuum extends Effect {
	...
	public void onRender(RenderContext context) {		
		...
		context.g2D.setComposite(RGBA_COMPOSITE);
		...
		context.g2D.setStroke(new BasicStroke(radius3));
		
		
		for(hz_band hz_band: hz_band) {
			context.g2D.setColor(
					new Color(
			...
			double
				...
				dx = amp * Math.cos(Math.toRadians(ang)),
				dy = amp * Math.sin(Math.toRadians(ang));
	...
	public void update(UpdateContext context) {
		...
		this.max_brightness = (double)Math.sqrt(this.max_brightness);
	...
	private static class hz_band implements Updateable {
		...
		public hz_band(Color rgba, double min_hz, double max_hz) {
			this.rgba = rgba;
			this.min_hz = min_hz;
			this.max_hz = max_hz;
			this.max_frq = .5f;
		...
		public void update(UpdateContext context) {
			...
			this.max_amp = 0;
			for(int i = a; i < b; i ++)
				if(context.mono[i] > this.max_amp) {
					this.max_amp = context.mono[i];
